...
| `##` section | Evidence | Screenshot id(s) |
|---|---|---|
| Standard reports (Overview, Conversation, CSAT, Agent, Label, Inbox, Bot) | Native report pages — `<!-- VERIFY-LIVE -->`; enriched by Proton sections added in `0020-reports-native-merge.patch` (`ProtonAgentsSection.vue`, `ProtonBotSection.vue`, `ProtonCsatSection.vue`). **Two corrections added during P8 task 10**, both because the tile labels claimed more than the data says: `v_resolution_split`'s `closed_by_bot`/`transfer_to_agent` are derived from Chatwoot `status` alone (`mapping.py::resolved_by`), so the Bot section's "bot-resolved %" and "bot-vs-agent split" mean *resolved* / *not yet resolved* and are NOT an AI-vs-human measure; and the CSAT section's NPS tiles are empty until `NPS_SAMPLE_RATE` is raised (before P8, `nps_score` was only reachable via the manual `POST /chat/nps`, never via the automatic survey). | ch07-standard-reports |
...
| Agent Availability & My Status | **Restored (v4)** — `0054-agent-status-selector.patch` (`presence.set_own_status`), `0062` (Add-status dialog), `0065-unify-availability-selector.patch` (profile-menu selector offers the catalogue). Backend `/routing/presence/*` mounted; env `PRESENCE_TRACKING_ENABLED=true`, `PRESENCE_CUSTOM_STATUSES_ENABLED=true`. `PRESENCE_THRESHOLD_ALERTS_ENABLED`/`ACW_ENABLED` absent → both off, stated as off in the chapter | ch09-my-status |
| ~~Workforce Dashboard~~ | **Restored (v4), then pulled back out the same day** — `0053-workforce-dashboard.patch` (`workforce.view`) is in the deployed image and `/admin/workforce` is mounted, but a browser session against the live tenant found the page shell loads and its data request never returns (no status code, no error, no timeout, 70+ seconds), observed twice; the public URL 401s in 0.37s unauthenticated, so routing/auth is not the fault. See `feature-guide-v3-pending.md`'s `## Workforce Dashboard` section | — |
