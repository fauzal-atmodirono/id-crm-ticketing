...
@pytest.mark.asyncio
async def test_carousel_with_empty_reply_uses_product_intro_not_error() -> None:
    """When show_models runs but the model emits no intro text, the user gets a
    product intro alongside the carousel — never the 'couldn't process' fallback."""
    settings = get_settings()

    def factory(_agent: Any) -> _CarouselRunner:
        # Empty reply text, but the carousel IS set (terminal tool call).
        return _CarouselRunner(
            reply="",
            session_service=svc._adk_sessions,
            session_id="carousel-empty",
            set_carousel=True,
        )
    ...
    result = await svc.handle_turn(session_id="carousel-empty", text="proton have other sedan?")

    assert result.reply == "Here are some models you might like:"
    assert result.reply != _EMPTY_REPLY_FALLBACK
