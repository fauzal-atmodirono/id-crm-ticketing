#!/usr/bin/env python3
"""Register the Knowledge Manager as a Chatwoot Dashboard App for one tenant.

Usage:
    python register_knowledge_app.py \
        --tenant default \
        [--app-url http://agent.1-2-3-4.nip.io/apps/knowledge-manager] \
        [--dry-run]
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="Register Knowledge Manager as a Chatwoot dashboard app.")
    ...
    p.add_argument("--app-url", required=True,
                   help="Full URL to the knowledge-manager app (e.g. http://agent.<ip>.nip.io/apps/knowledge-manager)")
    ...
    proton_backend_url = env.get("PROTON_BACKEND_URL") or os.environ.get("PROTON_BACKEND_URL", "")
    proton_backend_key = env.get("PROTON_BACKEND_KEY") or os.environ.get("PROTON_BACKEND_KEY", "")

    if not proton_backend_url or not proton_backend_key:
        print("ERROR: PROTON_BACKEND_URL and PROTON_BACKEND_KEY required.", file=sys.stderr)
        return 1

    app_url = (
        f"{args.app_url}?backend={quote(proton_backend_url, safe='')}&key={quote(proton_backend_key, safe='')}"
    )
    cfg = DashboardAppConfig(title="Knowledge", url=app_url)

    prefix = "[DRY RUN] " if args.dry_run else ""
    print(f"{prefix}Registering Knowledge Manager for '{args.tenant or chatwoot_url}'")
