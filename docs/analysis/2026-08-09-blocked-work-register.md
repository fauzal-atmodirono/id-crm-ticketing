...
> **UPDATE 2026-08-11 — "the patches cannot be verified here" is no longer
> true, and two of them were wrong.** Every row below that says a fork patch
> was "never verified against a real Chatwoot checkout" has now been checked
> for real. A throwaway container from the pinned upstream image
> (`chatwoot/chatwoot:v4.15.1`) git-inits `/app`, applies all 59 patches in
> order and **commits after each success**, so each patch meets the correctly
> accumulated tree. The run ends `=== FAILING:` with nothing after it: **all 59
> apply.** The container touches no tenant and no live stack.
>
> The first run of it found exactly two failures, both on
> `WootWriter/ReplyTopPanel.vue`: `0055` at line 12 and `0056` at line 97. Both
> were hand-written against a reconstructed import block that does not exist —
> they assumed a `useAlert` import (there is none) and put `useStore` where the
> real file has `useUISettings`. Both have been **regenerated from a real `git
> diff`** against the true post-0054 file, which is now committed as a test
> fixture (`deploy/chatwoot-fork/fixtures/ReplyTopPanel.post-0054.vue`). Full
> write-up: `.superpowers/sdd/fork-patch-verification.md`.
>
> This changes what these rows are waiting on, but does not empty them.
> **Applying is necessary, not sufficient:** none of these patches has been
> through a Cloud Build, so the vite build of the patched source is still
> unproven, and nothing here has been seen rendering in a browser. Every "do
> not demo before the build is green" instruction below stands unchanged.
...
| **Cloud Build for fork patch 0053** | P6's Workforce dashboard page. Hand-built (context transcribed from `0045`, script-verified hunk arithmetic), and **verified 2026-08-11 to apply cleanly to the real patched tree** in the container harness — the "never verified against a real Chatwoot checkout" caveat this row used to carry is discharged. The line-number-fix-up worry it described did not materialise. What remains is the build itself: no Cloud Build has compiled it, so vite is unproven. | `gcloud builds submit deploy/chatwoot-fork/ …` — off-VM, amd64. **Do not schedule a demo of the Workforce page before this build has gone green.** |
| **Cloud Build for fork patch 0054** | P6's agent-facing **My status** page (the availability-status picker + the administrator's status-catalogue editor). Same story as 0053: hand-built, hunk arithmetic script-generated, and **verified 2026-08-11 to apply cleanly to the real patched tree** in the container harness. Its three anchored hunks apply ON TOP OF 0053 and transcribe 0053's own added lines as context; since 0053 needed no fix-up, neither did this. The backend behind it (`features/routing/status_router.py`) is complete, needs no build, and **is now mounted** — `main.py` mounts `build_status_router` with the shared status/presence stores under `presence_custom_statuses_enabled`, and `test_p6_wiring.py` drives the endpoints through the real `bootstrap_application()` app to prove they answer rather than 404. So the API is live wherever that flag is on; only the page in front of it waits on this build. | `gcloud builds submit deploy/chatwoot-fork/ …` — off-VM, amd64. **Do not promise agents the "My status" page, or demo the catalogue editor, before this build has gone green.** Until then `PRESENCE_CUSTOM_STATUSES_ENABLED` on a tenant buys an API with no UI. |
| **Cloud Build for fork patch 0055** | P7 task 3's agent-facing **Translate** button in the reply composer's top panel (backs `translate_router.py`). **FAILED the first real check, and has been regenerated (2026-08-11).** The hand-built version did not apply: `patch failed: .../WootWriter/ReplyTopPanel.vue:12`. Its reconstruction of the import block was wrong — it assumed a `useAlert` import that does not exist anywhere in the file, and the line it thought was `useStore` is really `useUISettings`. The replacement is a real `git diff` against the true post-0054 file extracted from the pinned image, with `useAlert` added to the `dashboard/composables` import the file already has; no `@@` count was computed by hand. It **now applies cleanly** in the container harness, and its own automated guard is `test_p7_task7_faq_composer_patch.py`, which applies 0055 then 0056 to the committed ground-truth fixture. Still unbuilt: no Cloud Build has compiled it. | `gcloud builds submit deploy/chatwoot-fork/ …` — off-VM, amd64. **Do not demo the Translate button before this build has gone green.** The backend (`translate_router.py`) is independently testable and covered by its own suite regardless of this build; only the UI trigger waits on it. |
| **Cloud Build for fork patch 0057** | P9 tasks 2/3/6's **new-inbound alerting in the main Chatwoot UI** (toast/sound/desktop moved out of the `my-tasks` iframe) plus the **Alert preferences** page (backs `features/alerts/rules_router.py`). Hand-authored, and **verified 2026-08-11 to apply cleanly to the real patched tree** in the container harness (this row previously said it had never been checked against a real Chatwoot checkout). It was always the best-evidenced of the batch, which is presumably why it needed no correction: four of the six files are NEW (no upstream context at all), and the two modified files' hunks were **generated by `git diff`** — not written by hand — against a synthetic pre-image reconstructed only from lines transcribed verbatim from 0003's, 0025's, 0035's, 0041's, 0043's, 0053's and 0054's own merged diffs, then re-applied to a fresh copy of it with a real `git apply`. It **stacks on 0025** (Sidebar's `useProtonPermissions` import and `protonHasPermission` setup line), **0043** (the Cases nav block, its nav hunk's leading context) and **0003** (`...inboxRoutes,`). It deliberately anchors *below* 0053/0054's insertions rather than on their content, so a **content** change to those two cannot break it — only their line counts shift, which `git apply` absorbs as an offset. Two upstream APIs it could not read are **guesses**: the Vuex conversation-list getter names and `GET /api/v1/accounts/:id/conversations`'s response shape. Both fail toward a **visible degraded indicator**, never toward silence. | `gcloud builds submit deploy/chatwoot-fork/ …` — off-VM, amd64, **never on the prod VM, never from an arm64 Mac.** **Do not claim §3.1.1/§4.2 (pop-up on new inbound contact) before this build has gone green AND the manual screenshot check in 3h below has been done.** The preferences page's backend (`rules_router.py`) is independently tested and does not wait on this build — but it is **not mounted in `main.py` yet**, so today the page would 404 against a live backend. |
| **Upstream Chatwoot source** | This sandbox cannot reach github — but it does not need to. **Superseded 2026-08-11.** | The pinned upstream image already contains the source: `docker run --rm chatwoot/chatwoot:v4.15.1 sh -c 'cat /app/<path>'` on the deploy VM reads any upstream-owned file, and applying the patch stack inside that container yields the true *patched* pre-image for a new patch. `CustomAttributes.vue` is reachable this way. **Reconstructing upstream context from memory is now a mistake, not a constraint** — it is what made 0055 and 0056 fail. Extract the real file first (`deploy/chatwoot-fork/fixtures/` for anything a test needs), then generate the patch with a real `git diff`. |
...
| **Neither purge job can actually delete anything.** Both are now scheduled from `main.py` behind their own flags (2026-08-11) and both settings are real `Settings` fields, but the **destructive half of each is unwired on purpose**. | **The 90-day recording policy and the audit-retention policy are still not in force on any tenant** — what changed is that the gap is now measured and logged daily instead of being invisible. Audit: the audit-log port has no `delete` method and `AuditEntry` carries no document id, so the tick reads the real port, counts rows past the window and reports `dry_run`. Recordings: there is no Twilio recording-delete adapter and nothing that lists recordings due for purge, so the tick reports `not_executable` with **null** counts (null, not 0 — "nothing is past retention" is not something it can know). It also deliberately does **not** mark a recording deleted without a deleter: that would make the retrieval endpoint tell an agent "deleted under the retention policy" about audio that is still there. | Audit: a `delete` on `AuditLogPort` plus a stable row id (Firestore document id) exposed on `AuditEntry`. Recordings: a Twilio recording-delete adapter and a store query listing recordings by age, then `delete_func=`/`source=` at the two `main.py` call sites. Both are one main.py argument away, and neither should be turned on without watching one run. |
...
| **The deep health check probes only Postgres.** Mounted at `GET /healthz` (2026-08-11) and it can now fail — 503 when a probe fails or times out, bounded at 2.0s with probes run concurrently. But only the three Postgres engines (RBAC, pgvector KB, RSA) are actually probed. | Chatwoot, Firestore, Gemini/Vertex and Twilio are reported `unknown`/`not_probed` — never `ok`, which is the point — so `/healthz` cannot yet detect a Chatwoot or Firestore outage, and the overall status of a default tenant is `degraded` (200, deliberately not 503: an alert that fires because instrumentation is missing gets ignored). `GET /` is untouched and stays the container's liveness probe; a liveness probe that failed on a dependency outage would restart a healthy process in a loop. | One cheap authenticated call per remaining dependency (a Chatwoot profile GET, a Firestore `limit(1)`), plus a Cloud Monitoring uptime check pointed at `/healthz` rather than `/` — `docs/runbooks/monitoring-alerts.md` §5 item 1 still describes the old static-dict state and needs correcting |
...
1. ~~**The patch has never been through a Cloud Build or applied to a real
   Chatwoot checkout.**~~ **HALF-CLOSED 2026-08-11: it applies; it has still
   never been built.** Applied to a real checkout, the hand-written version
   failed — `patch failed: .../WootWriter/ReplyTopPanel.vue:97` — because it
   inherited `0055`'s wrong reconstruction of the import block, and because a
   later hand-edit to it had silently invalidated its own `@@` counts (the
   header claimed 96 insertions while the hunks added 107). It has been
   regenerated with a real `git diff` against the true post-0055 file and now
   applies cleanly in the container harness alongside all 59 other patches.
   Its tests (`backend/apps/backend/src/chatbot/test_p7_task7_faq_composer_
   patch.py`) no longer use a synthetic reconstruction at all: they apply
   `0055` then `0056` to the committed ground-truth pre-image
   (`deploy/chatwoot-fork/fixtures/ReplyTopPanel.post-0054.vue`). **Still
   owed: a Cloud Build.** Applying is not compiling, and no vite build has run
   over the patched source.
