...
replies to their own acknowledgement email rejoins their case too, as a
normal incoming message that reopens it. See the Conversations chapter's
Escalation replies section for the full walkthrough of what an agent sees
and does with it.
...
2. Apply a department label first (for example `dept_sales`), then the
   relevant dealer label if one is involved — see **Escalation Routing**
   in the Administration chapter for how these map to a specific PIC or
   dealer group.
