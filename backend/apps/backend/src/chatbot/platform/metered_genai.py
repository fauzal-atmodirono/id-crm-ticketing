...
SURFACE_CHAT_TRANSCRIBE = "chat.transcribe"
SURFACE_EMBED = "embed"
...
#
# The `/chat/turn` (and WhatsApp) reply is generated by google-adk from a model
# *string* -- `Agent(model=settings.gemini_model)` in `features/chat/agents.py`
# -- so ADK constructs its own `google.genai.Client` inside the installed
# package (`google/adk/models/google_llm.py`), which is not in this source tree
# and cannot be routed through this wrapper. `OrchestratorService`'s own client
# does the transcription/STT call on that path and nothing else, so it is
# labelled `chat.transcribe`, NOT `chat.turn`: a row labelled `chat.turn`
# carrying only a transcription's tokens would tell the cost report that a chat
# turn is nearly free, which is a wrong attribution rather than a missing one.
# The name is kept so that whoever meters ADK (an ADK `Gemini` subclass with an
# `api_client` property is the seam) has the label waiting, and so that the gap
# is recorded in code and not only in a report.
...
def _construct_raw_client(settings: Settings, *, fail_open: bool = True) -> Any | None:
    """Build the raw SDK client (ADC / Vertex). Fail-open by default: returns
    `None` when the SDK or credentials are unavailable, so wiring degrades
    rather than breaking boot.

    `fail_open=False` lets the SDK's own construction error propagate untouched
    (not even a log line of our own) instead of returning `None`. That exists
    for a call site whose **pre-P8 behaviour was to raise here**, and where
    swapping that for a `None` client would be a real regression rather than a
    kindness: `OrchestratorService.__init__` built its transcription client
    eagerly, so a missing `GOOGLE_API_KEY` failed the boot loudly (five
    bootstrap tests still assert this by needing the key to collect at all).
    Degrading it to `None` would move the failure to every voice turn, where it
    surfaces as an empty transcription -- a per-customer wrong answer instead of
    a deploy-time configuration error. Metering must not change when a call
    site discovers it is misconfigured.
...
def build_metered_genai_client(
    ...
    *,
    ...
    Args:
        ...
        fail_open: `True` (default) returns `None` when the SDK/credentials are
            unavailable. `False` re-raises the SDK's own construction error, for
            a call site that raised before P8 -- see `_construct_raw_client`.
            Ignored when `client` is injected: there is nothing to construct.
    ...
    Returns:
        ...
        constructed and `fail_open` (matching the previous per-site behaviour
        at each of the sites that fails open).
    """
    raw = client if client is not None else _construct_raw_client(settings, fail_open=fail_open)
    if raw is None:
        return None
