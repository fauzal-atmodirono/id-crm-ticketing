...
class OrchestratorService:
    ...
    def __init__(
        ...
        # The GenAI client for transcription/STT, routed through the token
        # metering wrapper so this path's spend reaches the cost report.
        #
        # Labelled `chat.transcribe`, deliberately NOT `chat.turn`: the reply on
        # this path is generated by google-adk from a model *string*
        # (`Agent(model=...)` in agents.py), so ADK builds its own
        # `google.genai.Client` inside the installed package, out of this tree
        # and out of reach of a client-boundary wrapper. This client performs
        # only `_transcribe_audio`'s call. Labelling it `chat.turn` would put a
        # transcription's handful of tokens under the name of the whole turn and
        # tell the cost report a chat turn is nearly free.
        #
        # `fail_open=False` preserves this constructor's pre-P8 behaviour
        # exactly: it raised the SDK's own error when credentials were missing,
        # failing the boot rather than degrading every voice turn to an empty
        # transcription. The type is `Any` because with metering on the object is
        # the wrapper's proxy, not the SDK class.
        self._genai_client: Any = build_metered_genai_client(
            settings, surface=SURFACE_CHAT_TRANSCRIBE, fail_open=False
        )
    ...
    async def _transcribe_audio(
        ...
        try:
            ...
            # `list[Content]` is valid input here. It used to need a
            # `type: ignore[arg-type]` because installing Pillow (via reportlab)
            # shifts google-genai's overload resolution; the client is now typed
            # `Any` (it is the metering proxy when the flag is on, not the SDK
            # class), so mypy no longer inspects the overloads and the ignore
