...
library so that `agent/`'s mirror of it can be loaded by file path from the
backend's test suite for a parity check (see `test_media_registry_parity.py`).
The two services have no shared package — CLAUDE.md is explicit that they
communicate only over HTTP — so the parity test is what stops the copies
drifting. Keep this module importable in isolation.

**Any edit here must be mirrored in `agent/app/services/media_registry.py`.**
"""
