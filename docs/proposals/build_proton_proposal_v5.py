#!/usr/bin/env python3
"""PROTON Technical Proposal deck — v4 (accelerated 2-month delivery plan).

Built from v3. The narrative sections (1 and 2) are unchanged; Section 3
"Delivery & Commercials" is rebuilt around the accelerated plan that the
commercials workbook
`PROTON - GCP Pricing, Timeline and PS Utilization (Accelerated 2-Month).xlsx`
prices, so deck and workbook tell the same story:

  - Project Timeline: 8 weeks (2 months), 5 phases, replacing the 12-16 week
    phased plan
  - New slide "How We Get to 8 Weeks" — what the accelerator supplies, and the
    honest counterpart: what it does NOT compress (Zendesk data fidelity, the
    SAML IdP integration, the IVR-4 root cause, client UAT time)
  - Effort Estimate: 136 base mandays + 20 contingency = 156, against 214/246
    on the standard-build plan
  - Scope framing: this engagement covers the work outside Packages A-G;
    A-G is already in delivery and carries its own estimate
  - GCP pricing is unchanged ($1,049/mo) — infrastructure carries the whole
    platform and is not split between the two scopes

--- inherited from v3 ---
PROTON Technical Proposal deck — v3 (client review-round revisions, deepened).

Built from v2, folding in the internal review-round feedback on that draft. This
pass goes further than the first v3 attempt — actual reordering, not just
relabeling:
  - Sections 2-4 merged AND reordered into one "Our Proposed Solution" flow,
    grouped by theme (The Platform -> Core Capabilities -> Built With You)
    instead of by build-phase (built / architecture / next), which read as
    three separate acts that looped back on themselves
  - Current State vs. Target State: icon-led row-by-row comparison table
    (was two paragraph-bullet cards) for a much stronger pros/cons pop
  - Data Migration: kept the 4-step flow, reframed as a "moving offices"
    analogy so it reads as plain-language reassurance, not a technical pipe
  - Google Program Funding: promoted to its own dedicated slide (was a small
    box crammed into Next Steps) — What it is / How it works / caveat

Sources folded in (same as v2):
...
diagrams (no embedded images). Output is a NEW file; v1 and v2 are left untouched.
"""
import os
...
OUT = os.path.join(BASE, "PROTON - Technical Proposal - Self-Managed CRM on Google Cloud (v5).pptx")
...
_DIV_TINT = {2: BEIGE, 3: BLUELT, 4: MINTLT}
...
textbox(s, Inches(0.7), Inches(3.9), Inches(8.6), Inches(0.4),
        [[("Prepared by Devoteam  ·  built to your 5-channel complaint SOP  ·  accelerated 2-month delivery", 12, False, WHITE)]])
textbox(s, Inches(0.7), Inches(4.33), Inches(8.6), Inches(0.4),
        [[("August 2026   ·   Version 5.0   ·   Demonstrated live 28 July 2026   ·   Confidential", 10, False, LIGHT)]])
...
# ---------- 7. DIVIDER §2 — OUR PROPOSED SOLUTION ----------
divider(2, "Our Proposed Solution", "The platform  →  core capabilities  →  built with you")

# ---------- 8. GCP PRODUCTION ARCHITECTURE (subsection: The Platform) ----------
s = slide()
page_header(s, "Section 2 · The Platform", "GCP Production Architecture")
...
# ---------- 9. CURRENT vs TARGET — icon-led comparison rows (subsection: The Platform) ----------
s = slide()
page_header(s, "Section 2 · The Platform", "Current State vs. Target State")
label_x, today_x, target_x = Inches(0.55), Inches(2.45), Inches(5.95)
label_w, today_w, target_w = Inches(1.9), Inches(3.5), Inches(3.5)
hy, hh = Inches(1.62), Inches(0.42)
rect(s, label_x, hy, label_w, hh, LIGHT)
textbox(s, label_x + Inches(0.1), hy, label_w - Inches(0.2), hh,
        [[("WHAT CHANGES", 10, True, DARK)]], anchor=MSO_ANCHOR.MIDDLE)
rect(s, today_x, hy, today_w, hh, POPPY)
textbox(s, today_x, hy, today_w, hh, [[("TODAY — ZENDESK (SaaS)", 12, True, WHITE)]],
        align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE)
rect(s, target_x, hy, target_w, hh, MINT)
textbox(s, target_x, hy, target_w, hh, [[("TARGET — SELF-MANAGED ON GCP", 12, True, WHITE)]],
        align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE)
compare_rows = [
    ("Licensing", "Per-agent subscription — pay per seat", "No seat fees — pay for infrastructure"),
    ("Data & AI", "Hosted by the vendor", "Inside PROTON's own GCP project"),
    ("Knowledge base", "Limited control over AI grounding", "Owned & edited by PROTON, no-code"),
    ("Integrations", "Bolted onto a closed platform", "Native DMS/TSP, telephony & BI"),
]
row_h = Inches(0.62); ry = Emu(int(hy) + int(hh)); icon_d = Inches(0.26)
for label, today_txt, target_txt in compare_rows:
    rect(s, label_x, ry, label_w, row_h, WHITE, line=LIGHT)
    textbox(s, label_x + Inches(0.12), ry, label_w - Inches(0.24), row_h,
            [[(label, 10.5, True, DARK)]], anchor=MSO_ANCHOR.MIDDLE, line_spacing=1.0)
    rect(s, today_x, ry, today_w, row_h, POPPYLT)
    icon_y = Emu(int(ry) + int(row_h)//2 - int(icon_d)//2)
    rect(s, today_x + Inches(0.14), icon_y, icon_d, icon_d, POPPY, shape=MSO_SHAPE.OVAL)
    textbox(s, today_x + Inches(0.14), icon_y, icon_d, icon_d, [[("✕", 11, True, WHITE)]],
            align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE)
    textbox(s, today_x + Inches(0.54), ry, today_w - Inches(0.68), row_h,
            [[(today_txt, 10, True, DARK)]], anchor=MSO_ANCHOR.MIDDLE, line_spacing=1.05)
    rect(s, target_x, ry, target_w, row_h, MINTLT)
    rect(s, target_x + Inches(0.14), icon_y, icon_d, icon_d, MINT, shape=MSO_SHAPE.OVAL)
    textbox(s, target_x + Inches(0.14), icon_y, icon_d, icon_d, [[("✓", 11, True, WHITE)]],
            align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE)
    textbox(s, target_x + Inches(0.54), ry, target_w - Inches(0.68), row_h,
            [[(target_txt, 10, True, DARK)]], anchor=MSO_ANCHOR.MIDDLE, line_spacing=1.05)
    ry = Emu(int(ry) + int(row_h))
textbox(s, Inches(0.55), Emu(int(ry) + int(Inches(0.14))), Inches(8.9), Inches(0.35),
...
# ---------- 10. LIVE DEMO RECAP (subsection: Core Capabilities) ----------
s = slide()
page_header(s, "Section 2 · Core Capabilities", "Demonstrated Live — Working Today")
...
# ---------- 11. OMNICHANNEL — LIVE CHANNEL STATUS (subsection: Core Capabilities) ----------
s = slide()
page_header(s, "Section 2 · Core Capabilities", "Omnichannel — Live Channel Status")
...
# ---------- 12. CONVERSATIONAL VOICE AI (subsection: Core Capabilities) ----------
s = slide()
page_header(s, "Section 2 · Core Capabilities", "Conversational Voice AI (Twilio)")
...
# handoff row
hy2 = Inches(3.1)
chip(s, Inches(0.55), hy2, Inches(3.0), Inches(0.7), "Escalate to human",
     "orchestrator — agent-B layer", DARK, tsize=11, ssize=8)
connector(s, Inches(3.55), hy2 + Inches(0.35), Inches(4.15), hy2 + Inches(0.35), color=POPPY)
routes = [("Sales", Inches(4.25)), ("Repair", Inches(6.05)), ("RSA — 24h", Inches(7.85))]
for name, x in routes:
    chip(s, x, hy2, Inches(1.55), Inches(0.7), name, "", BLUE, tsize=11)
...
# ---------- 13. AI & KNOWLEDGE (subsection: Core Capabilities) ----------
s = slide()
page_header(s, "Section 2 · Core Capabilities", "AI & Knowledge — Under Your Control")
...
# ---------- 14. LIFECYCLE & SLA MAPPED TO SOP (subsection: Core Capabilities) ----------
s = slide()
page_header(s, "Section 2 · Core Capabilities", "Lifecycle & SLA — Built to Your SOP")
...
# ---------- 15. CUSTOMER 360 & CASE MANAGEMENT (subsection: Built With You) ----------
s = slide()
page_header(s, "Section 2 · Built With You", "Customer 360 & Case Management")
...
# ---------- 16. EMAIL ESCALATION FLOW (subsection: Built With You) ----------
s = slide()
page_header(s, "Section 2 · Built With You", "Email Escalation — Your Priority Flow")
...
# ---------- 17. REPORTING & BI (subsection: Built With You) ----------
s = slide()
page_header(s, "Section 2 · Built With You", "Reporting & Business Intelligence")
...
# ---------- 18. GAP-CLOSURE PRIORITIES (subsection: Built With You) ----------
s = slide()
page_header(s, "Section 2 · Built With You", "Gap-Closure Priorities")
...
# ---------- 19. DATA MIGRATION — moving-offices analogy (subsection: Built With You) ----------
s = slide()
page_header(s, "Section 2 · Built With You", "Data Migration — Zendesk → Platform")
textbox(s, Inches(0.55), Inches(1.42), Inches(8.9), Inches(0.28),
        [[("Think of it like moving offices — pack up, label everything, move in, then unpack and get to work. "
           "Nothing gets left behind.", 10.5, False, MIDGREY, True)]], line_spacing=1.1)
flow = [("Pack everything up", "export every ticket, contact & FAQ from Zendesk", MIDGREY),
        ("Label it clearly", "match it to where it belongs in the new system", FIRE),
        ("Move it all in", "import into the new CRM", MINT),
        ("Unpack & set up", "ready to search, powered by AI", BLUE)]
fw = Inches(1.9); fh = Inches(0.8); fy = Inches(1.85); gap = Inches(0.32); fx = Inches(0.55)
for i, (t, d, c) in enumerate(flow):
    chip(s, fx, fy, fw, fh, t, d, c, tcolor=(DARK if c is FIRE else WHITE),
         scolor=(DARK if c is FIRE else WHITE), tsize=10.5, ssize=8.5)
    if i < len(flow) - 1:
        ax0 = Emu(int(fx) + int(fw))
        connector(s, ax0, fy + fh/2, Emu(int(ax0) + int(gap)), fy + fh/2, color=POPPY)
    fx = Emu(int(fx) + int(fw) + int(gap))
textbox(s, Inches(0.55), Inches(3.05), Inches(4.4), Inches(0.3), [[("WHAT MOVES ACROSS", 12, True, POPPY)]])
bullets(s, Inches(0.55), Inches(3.45), Inches(4.4), Inches(1.7),
        [("Every ticket & conversation ", "— with its status and labels kept intact"),
         ("Every customer record ", "— phone, email and details, exactly as they are today"),
         ("Your FAQ & knowledge base ", "— rebuilt as a smarter, searchable version PROTON owns"),
         ("Files & report history ", "— attachments and past reports come across too")],
        size=10, gap=6)
textbox(s, Inches(5.15), Inches(3.05), Inches(4.3), Inches(0.3), [[("NOTHING GETS LOST IN THE MOVE", 12, True, POPPY)]])
bullets(s, Inches(5.15), Inches(3.45), Inches(4.3), Inches(1.7),
        [("We pack copies first ", "— nothing is deleted from Zendesk during this step"),
         ("Old and new run side by side ", "— like living in both places until everything checks out"),
         ("Then we make it official ", "— once the data checks out, channels switch to the new system"),
         ("Zendesk switches off ", "— only after PROTON signs off that it's all correct")],
        size=10, gap=6, mcolor=MINT)
footer(s, npg())

# ---------- 20. DIVIDER §3 — DELIVERY & COMMERCIALS ----------
divider(3, "Scope of Work & Gap Analysis",
        "The Business Requirement, clause by clause — and exactly where we stand against it")

# ---------- SOW 1. OBJECTIVE & OVERALL GOALS ----------
s = slide()
page_header(s, "Section 3 · Scope of Work", "Business Requirement — Objective & Goals")
rect(s, Inches(0.55), Inches(1.42), Inches(8.9), Inches(1.38), BLUELT)
textbox(s, Inches(0.75), Inches(1.48), Inches(8.5), Inches(0.24), [[("OBJECTIVE", 10.5, True, BLUE)]])
textbox(s, Inches(0.75), Inches(1.74), Inches(8.5), Inches(1.0),
        [[("Build a smart, centralized complaint management system that combines all customer channels into one "
           "easy-to-manage platform, automatically assigns complaints to the right person, provides quick access to "
           "helpful information for faster responses, and tracks and monitors the complaint process in real time — "
           "improving customer satisfaction and enhancing the brand's service image.", 10, False, DARK)],
         [("Stated problem: the current system has limitations in integration, dashboards and lacks major functions.",
           9.5, True, POPPY)]], line_spacing=1.12, space_after=4)
goals = [("1  Unified Omni-Channel Management",
          "All customer messages — phone, email, chat or social media — brought together in one place for easier, more consistent handling"),
         ("2  Smart Task Assignment & Reminders",
          "Complaints automatically assigned to the right staff based on urgency and type, with reminders so nothing is missed"),
         ("3  Built-in Intelligent FAQ",
          "Staff can access an intelligent FAQ and help system to give quicker, more accurate answers to customers"),
         ("4  Automated Workflow & Escalation",
          "The complaint process runs automatically start to finish, with rules to escalate issues if there are delays or no response"),
         ("5  Real-Time Data & Performance Tracking",
          "Dashboards and reports show complaint trends, team performance and customer satisfaction to support better decisions"),
         ("6  Flexible and Scalable Design",
          "The system can grow and adapt to new business needs, more locations, or new customer channels in the future")]
gx = [Inches(0.55), Inches(3.55), Inches(6.55)]
for i, (title, body) in enumerate(goals):
    col, row = i % 3, i // 3
    x, y = gx[col], Inches(2.90 + row * 1.18)
    rect(s, x, y, Inches(2.9), Inches(1.10), MINTLT)
    textbox(s, x + Inches(0.14), y + Inches(0.06), Inches(2.62), Inches(0.32),
            [[(title, 9, True, MINT)]], line_spacing=1.0)
    textbox(s, x + Inches(0.14), y + Inches(0.40), Inches(2.62), Inches(0.64),
            [[(body, 7.5, False, DARK)]], line_spacing=1.06)
footer(s, npg())

# ---------- SOW 2. INBOUND + AGENT MANAGEMENT ----------
s = slide()
page_header(s, "Section 3 · Scope of Work", "A. Integration of Inbound   ·   B. Agent Management")
textbox(s, Inches(0.55), Inches(1.4), Inches(8.9), Inches(0.24),
        [[("A · INTEGRATION OF INBOUND", 10, True, POPPY)]])
rows = [["#", "Requirement"],
        ["A1", "All channels via Call, Email, WA or Social Media must be automatically integrated on the same agent interface & displayed in a single CRM view"],
        ["A2", "All new inbound will have a pop up notification (system alert)"],
        ["A3", "When received Call, system will have a voice recognition & change to text"]]
make_table(s, Inches(0.55), Inches(1.66), Inches(8.9), len(rows),
           [Inches(0.55), Inches(8.35)], rows,
           row_h=Inches(0.3), header_h=Inches(0.26), font=9, zebra_fill=BLUELT)
textbox(s, Inches(0.55), Inches(2.98), Inches(8.9), Inches(0.24),
        [[("B · AGENT MANAGEMENT", 10, True, POPPY)]])
rows = [["#", "Requirement"],
        ["B1", "CRM system able to check who is on duty before escalating to an agent"],
        ["B2", "The system must support setting channel priorities for each agent (for example, WhatsApp priority, Email priority, Call priority etc.)"],
        ["B3", "Automatic polling task assignment can be performed based on the current agent status (idle, busy, offline)"],
        ["B4", "If all agents in the priority channel are busy, the system can intelligently switch and assign tasks to the agents in the idle"],
        ["B5", "All tasks need to have \"Reminder\" and \"Timeout Warning\" functions, and support sound reminders, desktop reminders, and in-system notifications"]]
make_table(s, Inches(0.55), Inches(3.24), Inches(8.9), len(rows),
           [Inches(0.55), Inches(8.35)], rows,
           row_h=Inches(0.3), header_h=Inches(0.26), font=9, zebra_fill=MINTLT)
footer(s, npg())

# ---------- SOW 3. FAQ + ESCALATION ----------
s = slide()
page_header(s, "Section 3 · Scope of Work", "C. FAQ with AI Support   ·   D. Escalation")
textbox(s, Inches(0.55), Inches(1.4), Inches(8.9), Inches(0.22),
        [[("C · FAQ TEMPLATE WITH AI SUPPORT", 10, True, POPPY)]])
rows = [["#", "Requirement"],
        ["C1", "The system needs to integrate the FAQ knowledge base module to support the CRM team to update the FAQ content in real time"],
        ["C2", "During the conversation between agent and customer, the system can automatically match relevant FAQ entries based on chat keywords, and prompt real-time pop-up prompts to suggest related potential replies"],
        ["C3", "The agent can refer to the FAQ content with one click and can also provide feedback and score the FAQ quality"]]
make_table(s, Inches(0.55), Inches(1.64), Inches(8.9), len(rows),
           [Inches(0.55), Inches(8.35)], rows,
           row_h=Inches(0.28), header_h=Inches(0.24), font=9, zebra_fill=BLUELT)
textbox(s, Inches(0.55), Inches(2.82), Inches(8.9), Inches(0.22),
        [[("D · ESCALATION PROCESS", 10, True, POPPY)]])
rows = [["#", "Requirement"],
        ["D1", "Rule engine, according to the classification / categorization of customer problems (department / business area) combined with SOP, to automatically identify the Person In Charge (PIC)"],
        ["D2", "Automatically notify and CC the relevant personnel by email (can support attachments if needed — photos, videos, web link etc.)"],
        ["D3", "Trigger a WhatsApp message to alert the PIC (optional)"],
        ["D4", "Escalate triggers an automatic reminder of the higher-level responsible person"],
        ["D5", "Records of WIP and Resolved operations"],
        ["D6", "All status changes need to record information such as Agent name, time, and remarks"],
        ["D7", "Automatic escalation mechanisms can be configured, such as 8-hours automatic send to higher level if there is no response"],
        ["D8", "48-hours unresolved alarms"]]
make_table(s, Inches(0.55), Inches(3.06), Inches(8.9), len(rows),
           [Inches(0.55), Inches(8.35)], rows,
           row_h=Inches(0.235), header_h=Inches(0.22), font=8.5, zebra_fill=MINTLT)
footer(s, npg())

# ---------- SOW 4. CUSTOMER PROFILE ----------
s = slide()
page_header(s, "Section 3 · Scope of Work", "E. Customer Profile — Customer 360")
textbox(s, Inches(0.55), Inches(1.38), Inches(8.9), Inches(0.24),
        [[("E · CUSTOMER PROFILE", 10, True, POPPY)]])
rows = [["#", "Requirement"],
        ["E1", "The system needs to realize two-way data connection with the DMS system (Dealer Management System) and the TSP platform (Telematics Service Provider)"],
        ["E2", "When the customer contacts the call center through the hotline or WhatsApp, the system should automatically identify the customer and display their key information"],
        ["E3", "Trigger condition — customer contacts system by phone or WhatsApp (based on caller number or WA number)"],
        ["E4", "The system calls the DMS and TSP systems through API interfaces to automatically match and pull relevant information"],
        ["E5", "Customer Information — name, gender, contact detail, address etc."],
        ["E6", "Vehicle Information — VRN, VIN, Model, Dealer Purchased, Purchased date etc."],
        ["E7", "Service History — Recent Service Date, Service Dealer, Summary of Repair Record, Status of Open RO"],
        ["E8", "Call Center History — all call history, WA communication history, emails & any social media reply or Road Side Assistance"],
        ["E9", "After a customer calls, the Agent page automatically pops up the Customer 360 View Card"],
        ["E10", "The content of the card is divided into personal information, vehicle information, service information, and Call Center history"],
        ["E11", "All data synchronization should take no more than 3 seconds and support asynchronous loading"]]
make_table(s, Inches(0.55), Inches(1.62), Inches(8.9), len(rows),
           [Inches(0.6), Inches(8.3)], rows,
           row_h=Inches(0.29), header_h=Inches(0.26), font=9, zebra_fill=BLUELT)
footer(s, npg())

# ---------- SOW 5. REPORTING ----------
s = slide()
page_header(s, "Section 3 · Scope of Work", "F. Reporting & Business Intelligence")
textbox(s, Inches(0.55), Inches(1.42), Inches(8.9), Inches(0.22),
        [[("F · REPORTING  —  integrate Power BI (or equivalent BI tools) to implement the following data visualization reports", 9.5, True, POPPY)]])
left = [["#", "Requirement"],
        ["F1", "Power BI or equivalent BI tool for data visualization"],
        ["F2", "Channel source analysis by case (Call / WA / Email / Social Media)"],
        ["F3", "By case division (Apps, Sales, Aftersales, Charging)"],
        ["F4", "Daily / weekly / monthly trend chart"],
        ["F5", "Case distribution and resolution statistics by dealer / department / PIC"],
        ["F6", "Ranking of first response rate, resolution rate and resolution time of each department / dealer"],
        ["F7", "Analysis of the Complaint Reopen Rate, CRR by dealers / department / PIC"],
        ["F8", "SLA response achievement rate"],
        ["F9", "Number of tasks processed by each agent and the average response time"]]
right = [["#", "Requirement"],
         ["F10", "Average response duration (first response time) and average case closure time, based on channel response time"],
         ["F11", "Ranking of popular complaint types and statistics during peak complaint hours"],
         ["F12", "NPS — customer able to rate the agent performance from Call & WA channel"],
         ["F13", "Case life cycle — a map of the time distribution from creation to closing"],
         ["F14", "Trend analysis of special states such as Higher escalation / WIP / Temporary Closed / Closed"],
         ["F15", "Reports exported to PDF, Excel, and automatically sent to management on a regular basis"],
         ["F16", "Configurable \"Anomaly Warning Dashboard\" for real-time anomaly prompts (such as the explosion of a channel)"],
         ["F17", "System should be able to run report based on request tag key work"]]
make_table(s, Inches(0.55), Inches(1.62), Inches(4.35), len(left),
           [Inches(0.5), Inches(3.85)], left,
           row_h=Inches(0.37), header_h=Inches(0.24), font=8.5, zebra_fill=BLUELT)
make_table(s, Inches(5.1), Inches(1.62), Inches(4.35), len(right),
           [Inches(0.55), Inches(3.8)], right,
           row_h=Inches(0.42), header_h=Inches(0.24), font=8.5, zebra_fill=BLUELT)
footer(s, npg())

# ---------- GAP 1. SCORECARD ----------
s = slide()
page_header(s, "Section 3 · Gap Analysis", "Where the Platform Stands — 47 Requirements Scored")
textbox(s, Inches(0.55), Inches(1.42), Inches(8.9), Inches(0.26),
        [[("Every clause of the BRD, assessed against what exists in the platform today — not against what is planned.",
           9.5, False, MIDGREY, True)]])
rows = [["Requirement area", "Reqs", "Delivered", "Partial", "Gap", "Headline position"],
        ["A · Integration of Inbound", "3", "2", "1", "—",
         "Call, Email, WA, Web & API live in one view; Facebook/Instagram not yet connected"],
        ["B · Agent Management", "5", "4", "1", "—",
         "On-duty check, channel priorities, status-based assignment & idle overflow all shipped"],
        ["C · FAQ Template with AI Support", "3", "2", "1", "—",
         "Knowledge module live with real-time editing and AI-suggested replies"],
        ["D · Escalation Process", "8", "7", "1", "—",
         "PIC rule engine, email + CC, WA alert, WIP/Resolved states, 8h/48h timers all shipped"],
        ["E · Customer Profile (Customer 360)", "11", "2", "3", "6",
         "The one material gap — DMS and TSP integration does not exist"],
        ["F · Reporting & BI", "17", "17", "—", "—",
         "Complete: BigQuery warehouse, all listed views, exports, scheduling and anomaly alerting"],
        ["Total", "47", "34", "7", "6", "72% delivered · 15% partial · 13% gap — and every gap sits in one area"]]
tbl = make_table(s, Inches(0.55), Inches(1.76), Inches(8.9), len(rows),
                 [Inches(2.25), Inches(0.5), Inches(0.85), Inches(0.6), Inches(0.45), Inches(4.25)], rows,
                 row_h=Inches(0.42), header_h=Inches(0.3), font=9, zebra_fill=LIGHT)
...
# ---------- GAP 2. PARTIALS ----------
s = slide()
page_header(s, "Section 3 · Gap Analysis", "Partially Met — What Is Missing, and What Closes It")
textbox(s, Inches(0.55), Inches(1.42), Inches(8.9), Inches(0.26),
        [[("Seven clauses where the capability is shipped but a bounded piece remains. None is a redesign.",
           9.5, False, MIDGREY, True)]])
rows = [["#", "What is already working", "What is still missing", "How it closes"],
        ["A1", "WhatsApp, Email, Web, API and Voice all land in one Chatwoot agent view",
         "Facebook / Instagram inboxes not connected", "HTTPS on the tenant host + Meta Business verification — configuration, not build"],
        ["B5", "Reminder, timeout warning, live countdown and WhatsApp nudge near breach",
         "Sound and desktop cues rely on Chatwoot-native notifications; custom agent status labels not built",
         "Native notifications configured at handover; status labels are a small build"],
        ["C3", "FAQ feedback and 👍/👎 quality scoring shipped",
         "\"One click\" insert is copy-to-clipboard, not direct insertion",
         "Chatwoot iframe sandbox limitation — needs a fork change to the composer"],
        ["D2", "Escalation email to the PIC with per-department CC shipped",
         "Attachments (photos, videos, links) not carried on the escalation email",
         "Wire conversation attachments into the notifier — small, needs a source decision"],
        ["E2", "Contact identified by caller / WA number via Chatwoot contact matching",
         "No enrichment from an external system of record", "Follows E1 — DMS/TSP API access"],
        ["E5", "Name and contact details held on the Chatwoot contact",
         "Gender, address and the authoritative customer record live in the DMS", "Follows E1 — DMS/TSP API access"],
        ["E10", "360 card exists with personal-information and call-centre-history sections",
         "Vehicle and service sections have no data to render", "Follows E1 — DMS/TSP API access"]]
make_table(s, Inches(0.55), Inches(1.76), Inches(8.9), len(rows),
           [Inches(0.45), Inches(2.75), Inches(2.75), Inches(2.95)], rows,
           row_h=Inches(0.44), header_h=Inches(0.28), font=8, zebra_fill=BLUELT)
footer(s, npg())

# ---------- GAP 3. THE REAL GAP ----------
s = slide()
page_header(s, "Section 3 · Gap Analysis", "The One Real Gap — Customer 360 / DMS & TSP")
textbox(s, Inches(0.55), Inches(1.4), Inches(8.9), Inches(0.26),
        [[("All six unmet clauses sit in section E, and all six have the same root cause: there is no DMS or TSP "
           "integration, because we do not yet have API access or a data contract.", 9.5, False, MIDGREY, True)]])
rows = [["#", "Requirement not met", "Status today", "Dependency"],
        ["E1", "Two-way data connection with DMS and TSP", "No DMS/TSP client exists in the platform", "API specification + credentials from PROTON"],
        ["E4", "Call DMS and TSP through API to match and pull information", "No integration layer built", "API specification + credentials"],
        ["E6", "Vehicle Information — VRN, VIN, Model, Dealer, Purchase date", "Vehicle number captured for reporting; no vehicle master data", "DMS vehicle master"],
        ["E7", "Service History — service date, dealer, repair record, open RO", "Not available anywhere in the platform", "DMS service/RO records"],
        ["E9", "Agent page automatically pops up the 360 card on an inbound call", "Lookup exists as a manual search by phone or vehicle number", "Follows E1; then a screen-pop trigger"],
        ["E11", "Data synchronization within 3 seconds, asynchronous loading", "Cannot be designed or measured until the integration exists", "Follows E1 — then a performance target"]]
make_table(s, Inches(0.55), Inches(1.74), Inches(8.9), len(rows),
           [Inches(0.5), Inches(2.7), Inches(3.1), Inches(2.6)], rows,
           row_h=Inches(0.4), header_h=Inches(0.28), font=8.5, zebra_fill=POPPYLT)
rect(s, Inches(0.55), Inches(4.62), Inches(8.9), Inches(0.6), BEIGE)
textbox(s, Inches(0.75), Inches(4.62), Inches(8.5), Inches(0.6),
        [[("What we are building regardless — ", 9.5, True, POPPY),
          ("the integration surface: a configurable DMS/TSP connection in Settings, a defined client interface, and the "
           "360 card already consuming it. When PROTON supplies the API specification, the adapter plugs in behind it.",
           9.5, False, DARK)]], anchor=MSO_ANCHOR.MIDDLE, line_spacing=1.08)
footer(s, npg())

divider(4, "Delivery & Commercials", "Timeline · Scope · Deliverables · Engagement")

# ---------- 21. PROJECT TIMELINE ----------
s = slide()
page_header(s, "Section 4", "Project Timeline — 8 Weeks (2 Months), Accelerated")
rows = [["Phase", "Wk", "Key tasks"],
        ["P1 — Assessment\n& planning", "1",
         "Kick-off & scope confirmation vs BRD  ·  validate the 5-channel SOP and escalation matrix  ·  solution architecture, landing-zone design & migration plan"],
        ["P2 — GCP foundation\n& provisioning", "1–2",
         "GCP project, VPC, IAM & landing zone from accelerator IaC  ·  GKE Standard + node pools, Cloud SQL (HA), Memorystore, BigQuery  ·  CI/CD, Secret Manager, monitoring baseline"],
        ["P3 — Core platform\ncutover", "2–4",
         "Deploy Chatwoot + agent + backend via Helm charts  ·  migrate Zendesk tickets, contacts, history & attachments  ·  migrate KB  ·  wire WhatsApp / Web / API / Email  ·  AI + SOP lifecycle flows live"],
        ["P4 — Gap closure", "4–5",
         "Email escalation (auto-ack + internal case)  ·  Customer 360 + DMS/TSP surface  ·  hierarchical categories, per-case dashboard, RBAC  ·  reporting & BI  ·  agent management  ·  bulk FAQ upload"],
        ["P5 — Hardening,\nHA & UAT", "5–7",
         "HA / autoscaling & tuning  ·  monitoring, alerting & logging  ·  backup/DR with restore test  ·  security & RBAC review  ·  live channel verification  ·  UAT, reconciliation & sign-off"],
        ["P6 — Cutover, handover\n& training", "7–8",
         "Cutover, go-live & hypercare  ·  Zendesk decommission  ·  ops runbook  ·  admin & agent training  ·  knowledge transfer & closure"]]
make_table(s, Inches(0.55), Inches(1.5), Inches(8.9), len(rows),
           [Inches(1.75), Inches(0.5), Inches(6.65)], rows,
           row_h=Inches(0.5), header_h=Inches(0.28), font=8.5, zebra_fill=BLUELT)
textbox(s, Inches(0.55), Inches(4.86), Inches(8.9), Inches(0.4),
        [[("36 working days of duration across 8 calendar weeks — phases overlap, with gap closure starting while the "
           "platform cutover settles. Assumes PROTON supplies the Zendesk export, DMS/TSP API access and UAT participants on schedule.",
           9, False, MIDGREY, True)]])
footer(s, npg())

# ---------- 21b. HOW WE GET TO 8 WEEKS ----------
s = slide()
page_header(s, "Section 4", "How We Get to 8 Weeks")
textbox(s, Inches(0.55), Inches(1.44), Inches(8.9), Inches(0.28),
        [[("Against a 16-week standard build of the same scope: 252 → 120 mandays. The compression is the accelerator, "
           "not a thinner scope.", 9.5, False, MIDGREY, True)]])
rect(s, Inches(0.55), Inches(1.82), Inches(4.35), Inches(2.9), MINTLT)
textbox(s, Inches(0.75), Inches(1.94), Inches(4.0), Inches(0.3),
        [[("WHAT THE ACCELERATOR SUPPLIES", 11.5, True, MINT)]])
bullets(s, Inches(0.75), Inches(2.34), Inches(3.95), Inches(2.3),
        [("Landing zone as IaC ", "— GCP project, VPC/IAM, GKE, Cloud SQL parameterised, not authored"),
         ("Helm deployment ", "— Chatwoot, agent and backend ship as charts"),
         ("Migration tooling ", "— Zendesk extract, mapping and bulk import already built"),
         ("HA / observability / DR ", "— applied as baselines rather than designed per project"),
         ("Scripted release ", "— image rebuild, per-tenant flags and BigQuery views run as pipeline steps")],
        size=10, gap=7, mcolor=MINT)
rect(s, Inches(5.1), Inches(1.82), Inches(4.35), Inches(2.9), POPPYLT)
textbox(s, Inches(5.3), Inches(1.94), Inches(4.0), Inches(0.3),
        [[("WHAT IT DOES NOT COMPRESS", 11.5, True, POPPY)]])
bullets(s, Inches(5.3), Inches(2.34), Inches(3.95), Inches(2.3),
        [("Zendesk data fidelity ", "— your data shape is client-specific; mapping and reconciliation stay full-effort"),
         ("DMS / TSP integration ", "— depends on API access and data contracts we don't hold yet"),
         ("UAT ", "— bounded by your team's availability, not by our tooling"),
         ("Cutover & training ", "— hypercare and knowledge transfer are calendar time, not build time")],
        size=10, gap=9, mcolor=POPPY)
rect(s, Inches(0.55), Inches(4.78), Inches(8.9), Inches(0.44), BEIGE)
textbox(s, Inches(0.75), Inches(4.78), Inches(8.5), Inches(0.44),
        [[("The trade-off — ", 10, True, POPPY),
          ("with no contingency in the estimate, slippage on your inputs moves the go-live date rather than being absorbed.",
           10, False, DARK)]],
...
# ---------- 22. SCOPE OF WORK ----------
s = slide()
page_header(s, "Section 4", "Delivery Scope — What We Build & Run")
...
bullets(s, Inches(0.55), Inches(1.5), Inches(4.45), Inches(3.4), sc1, size=9.5, gap=5)
bullets(s, Inches(5.15), Inches(1.5), Inches(4.3), Inches(3.4), sc2, size=9.5, gap=5)
textbox(s, Inches(0.55), Inches(4.62), Inches(8.9), Inches(0.56),
        [[("Assumptions:  ", 8.5, True, POPPY),
          ("PROTON provides GCP billing/org access, DMS/TSP API + data contracts, email domain, Zendesk export, UAT participants and a telephony/CTI decision.",
           8.5, False, DARK)],
         [("Delivery model:  ", 8.5, True, POPPY),
          ("accelerator-led — 120 mandays over 36 working days, inside a 2-month calendar.",
           8.5, False, DARK)]], line_spacing=1.08, space_after=2)
footer(s, npg())

# ---------- 23. OUT OF SCOPE ----------
s = slide()
page_header(s, "Section 4", "Out of Scope")
...
# ---------- 24. DELIVERABLES ----------
s = slide()
page_header(s, "Section 4", "Deliverables")
...
# ---------- 25. EFFORT & MANDAYS ----------
s = slide()
page_header(s, "Section 4", "Effort Estimate — Mandays")
textbox(s, Inches(0.55), Inches(1.42), Inches(8.9), Inches(0.26),
        [[("Accelerated 2-month engagement — 120 mandays across 36 working days of duration. Mandays per role and phase; "
           "day-rates in the commercial workbook.", 9.5, False, MIDGREY, True)]])
rows = [["Phase", "Days", "PM", "SA", "BE/AI", "FE", "BI", "QA", "MD"],
        ["P1 — Assessment & planning", "3", "2", "3", "3", "0", "0", "0", "8"],
        ["P2 — GCP foundation & provisioning", "3", "2", "3", "3", "1", "2", "1", "12"],
        ["P3 — Core platform cutover", "10", "5", "1", "10", "10", "2", "2", "30"],
        ["P4 — Gap closure", "5", "5", "3", "5", "5", "2", "2", "22"],
        ["P5 — Hardening, HA & UAT", "10", "5", "2", "10", "5", "5", "5", "32"],
        ["P6 — Cutover, handover & training", "5", "4", "5", "5", "0", "2", "0", "16"],
        ["Total — accelerated delivery effort", "36", "23", "17", "36", "21", "13", "10", "120"]]
tbl = make_table(s, Inches(0.55), Inches(1.74), Inches(8.9), len(rows),
                 [Inches(3.2), Inches(0.65), Inches(0.7), Inches(0.7), Inches(0.8),
                  Inches(0.65), Inches(0.65), Inches(0.65), Inches(0.9)], rows,
                 row_h=Inches(0.3), header_h=Inches(0.3), font=9.5, zebra_fill=LIGHT)
for ri in range(len(rows)):
    for ci in range(1, 9):
        for p in tbl.cell(ri, ci).text_frame.paragraphs:
            p.alignment = PP_ALIGN.CENTER
...
for ci in range(9):
    ...
    for p in cell.text_frame.paragraphs:
        for r in p.runs:
            r.font.bold = True; r.font.color.rgb = DARK
textbox(s, Inches(0.55), Inches(4.18), Inches(8.9), Inches(0.75),
        [[("Against the standard build:  ", 9.5, True, POPPY),
          ("252 mandays over 16 weeks → 120 over 8 weeks. Same scope, accelerator-led delivery.", 9.5, False, DARK)],
         [("Commercials:  ", 9.5, True, POPPY),
          ("Rp 548,450,000 in mandays → Rp 630,717,500 including margin.", 9.5, False, DARK)],
         [("No contingency is carried ", 9.5, True, POPPY),
          ("— the plan depends on PROTON supplying the Zendesk export, DMS/TSP API access and UAT participants on schedule.",
           9.5, False, DARK)]], line_spacing=1.1, space_after=3)
textbox(s, Inches(0.55), Inches(4.98), Inches(8.9), Inches(0.26),
        [[("Team:  ", 8.5, True, POPPY),
          ("PM · Solution Architect / Tech Lead · Backend/AI · Frontend · Data/BI · QA — Solution Architect at the Lead Consultant rate.",
           8.5, False, DARK)]])
footer(s, npg())

# ---------- 26. GCP PRICING & SPECIFICATION ----------
s = slide()
page_header(s, "Section 4", "GCP Pricing & Specification")
...
# ---------- 27. GOOGLE PROGRAM FUNDING — dedicated slide ----------
s = slide()
page_header(s, "Section 4", "Google Program Funding — Exploratory")
rect(s, Inches(0.55), Inches(1.62), Inches(4.35), Inches(2.7), BLUELT)
textbox(s, Inches(0.75), Inches(1.74), Inches(4.0), Inches(0.3), [[("WHAT IT IS", 11.5, True, BLUE)]])
bullets(s, Inches(0.75), Inches(2.14), Inches(3.95), Inches(2.1),
        [("Partner-backed programs ", "— Google runs funding & credit programs for customers modernizing on Google Cloud"),
         ("Could offset cost ", "— development and/or cloud infrastructure spend for this engagement"),
         ("Google's call ", "— availability and amount depend on Google's own review, not Devoteam's")],
        size=10.5, gap=10, mcolor=BLUE)
rect(s, Inches(5.1), Inches(1.62), Inches(4.35), Inches(2.7), MINTLT)
textbox(s, Inches(5.3), Inches(1.74), Inches(4.0), Inches(0.3), [[("HOW IT WORKS", 11.5, True, MINT)]])
bullets(s, Inches(5.3), Inches(2.14), Inches(3.95), Inches(2.1),
        [("Caroline opens the door ", "— starts the conversation with Google, in parallel with this proposal"),
         ("We support if asked ", "— Devoteam provides technical scoping / use-case details on request"),
         ("Your role ", "— confirm interest and share any account details Google's team needs")],
        size=10.5, gap=10, mcolor=MINT)
rect(s, Inches(0.55), Inches(4.5), Inches(8.9), Inches(0.55), BEIGE)
textbox(s, Inches(0.75), Inches(4.5), Inches(8.5), Inches(0.55),
        [[("Exploratory only — ", 10.5, True, POPPY),
          ("not guaranteed, and this proposal's scope, timeline and pricing do not depend on it.", 10.5, False, DARK)]],
        anchor=MSO_ANCHOR.MIDDLE)
footer(s, npg())

# ---------- 28. ENGAGEMENT & COMMERCIALS ----------
s = slide()
page_header(s, "Section 4", "Engagement & Next Steps")
...
bullets(s, Inches(0.55), Inches(1.98), Inches(4.5), Inches(2.9),
        [("One tailored proposal ", "— Devoteam compiles & tailors all your requirements (target: next week)"),
         ("Parallel Google track ", "— Caroline opens the solution discussion with Google (funding + delivery — see previous page)"),
        ...
        size=11, gap=11)
rect(s, Inches(5.15), Inches(1.62), Inches(4.3), Inches(2.66), POPPYLT)
textbox(s, Inches(5.35), Inches(1.74), Inches(3.9), Inches(0.3), [[("KNOWN DELTAS BEING CLOSED", 11.5, True, POPPY)]])
bullets(s, Inches(5.35), Inches(2.14), Inches(3.9), Inches(2.0),
...
# ---------- 29. WHY THIS WORKS (dark closing) ----------
s = slide()
...
bullets(s, Inches(0.75), Inches(1.82), Inches(8.5), Inches(2.6),
        [("Proven live — ", "demonstrated against your SOP on 28 Jul 2026: migration + hardening, not invention"),
         ...
         ("Cost-down, capability-up — ", "managed infrastructure you scale on your terms, with Google funding being explored in parallel")],
