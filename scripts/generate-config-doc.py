...
FLAG_CAVEATS = [
    ...
    "404 at every value of this setting. **It is now mounted** (review fix "
    "`1fc11a2`), and `test_p11_wiring.py` proves reachability through the real "
    "app by asserting the discriminating pair: 401 rather than 404 "
    "unauthenticated, and the flag's own 404 for a caller who *would* be "
    "permitted — the only way to tell \"flag off\" apart from \"unmounted\". "
    "**The substantive limit survives the mount:** the handler reads an "
    "in-process registry (`_RECORDING_RETENTIONS`) that nothing in production "
    "writes to, so against a real conversation it answers \"no recording "
    "exists\" regardless of this setting. The Chatwoot custom-attribute read "
    "that would populate it is owed.",
    "**`DATA_SCOPED_RBAC_ENABLED` has no consumer at all.** This is the one "
    "flag in this document that gates nothing whatsoever: "
    "`apply_scope_to_filters` and `resolve_user_data_scope` have no callers, "
    "there is no FastAPI dependency wiring them in, and role scopes live in a "
    "module dict with no admin surface. The scope-intersection logic is real "
    "and proven; **the enforcement is absent**, so setting this flag to `true` "
    "restricts nothing and no data scope is applied to any query. Earlier text "
    "in `config.py` and `example.env` described enforcement \"in the metrics "
    "and admin query layers plus Chatwoot-native inbox membership\" — that was "
    "an overclaim and has been corrected in all three places. See the "
    "blocked-work register §3j.",
    "**Three `PHONE_*` settings gate nothing today.** "
    "`PHONE_DTMF_MENU_ENABLED` and `PHONE_LIVE_TRANSCRIPT_ENABLED` have no "
    "consumer, and the recording-retention job has no scheduler, so "
    "`PHONE_RECORDING_RETENTION_DAYS` and `PHONE_RETENTION_JOB_ENABLED` do not "
    "cause anything to be deleted. Each field's comment now says so. See the "
    "blocked-work register §3k.",
