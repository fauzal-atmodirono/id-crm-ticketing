...
9. Below the persona panel, **Tenant settings** lets an administrator
   ...
   - **Inbound auto-acknowledgement template** — the message sent the
     first time a new email thread arrives, when the inbound
     auto-acknowledgement is turned on for the tenant.
   - **Escalation acknowledgement template** — the message sent to the
     customer as part of the two-thread escalation email (see the AI
     Assistant Behaviour chapter's Escalation labels & the escalation
     email section).

   Leaving either template blank keeps the platform's built-in default
   wording, exactly like the other fields on this panel. Editing one only
   changes the wording it sends — it doesn't turn the underlying feature
   on if it's currently off for your tenant.
