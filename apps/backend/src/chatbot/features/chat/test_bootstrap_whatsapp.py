...
def test_sla_audit_wiring_end_to_end() -> None:
    ...
    A unique ticket_id is generated per run so the test is hermetic against
    persistent backends (e.g. Firestore when HANDOFF_STORE=firestore in .env).
    """
    app = bootstrap_application()
