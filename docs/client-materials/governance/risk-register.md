...
## R2 — Every upstream Chatwoot security release requires rebasing 59 fork patches
...
The Chatwoot SPA is forked. Customisation is delivered as **59 patch files**
(`deploy/chatwoot-fork/patches/0001-*` through `0060-*`; `0042` is absent) that
are `git apply`-ed onto upstream at image-build time. Every upstream version bump
— including one taken purely for a CVE — means re-applying all 59, fixing
...
design was written and is 59 now; ten patches were added by this programme
...
## R7 — Nine fork patches, now applied to a real Chatwoot checkout; none yet built

| | |
|---|---|
| **Likelihood** | **Reduced 2026-08-11.** All 59 patches now apply to the real upstream tree; two needed rework and got it. Residual risk is the frontend build, not the patches |
| **Impact** | Medium — each is a UI feature that does not exist until its build succeeds |
| **Status** | **Open, narrowed.** Blocked on a Cloud Build run |
| **Owner** | Engineering, needs a Cloud Build run |

**Updated 2026-08-11 — the central claim of this row has been tested, and it was
right.** Patches **0052 through 0060** were authored without access to the real
Chatwoot source (no network to clone upstream), each against a *synthetic*
reconstruction of its context. This row predicted "high likelihood at least one
needs rework". They have now been applied for real: the pinned upstream image
already contains the source, so a throwaway container built from
`chatwoot/chatwoot:v4.15.1` can git-init the tree, apply all 59 patches in order
and commit after each one. **Two failed** — `0055` and `0056`, both on
`WootWriter/ReplyTopPanel.vue`, both because the reconstruction assumed a
`useAlert` import that does not exist and mis-identified the line that is really
`useUISettings`. Both were regenerated from a real `git diff` against the true
file. **The current patch set applies cleanly, end to end, with no failures.**

The container runs from the upstream base image and touches no tenant.

| Patch | Feature | Verification quality |
|---|---|---|
| `0052` | Escalation tier-2 manager contact | Applies cleanly to the real tree |
| `0053` | P6 Workforce dashboard page | Was the weakest-evidenced (context transcribed from `0045` from memory). Applies cleanly to the real tree |
| `0054` | P6 agent "My status" page | Stacks on `0053`. Applies cleanly to the real tree |
| `0055` | P7 Translate button | **FAILED (`ReplyTopPanel.vue:12`), regenerated from a real `git diff`.** Now applies cleanly |
| `0056` | P7 FAQ composer strip | **FAILED (`ReplyTopPanel.vue:97`), regenerated from a real `git diff`.** Stacks on `0055`; its arithmetic had also broken once before (80 vs 107 added lines) after a hand edit. Now applies cleanly |
| `0057` | P9 inbound alerts + preferences page | Best-evidenced of the batch; 4 of 6 files new, hunks generated by real `git diff`, tests execute the shipped JavaScript in Node. Applies cleanly |
| `0058` | Feature-flag unification | Hunks generated by `git diff`; tests render the shipped ERB through Ruby's own `erb`. Applies cleanly |
| `0059` | Roles & Permissions redesign | Pre-image reconstructed by replaying `0027`→`0028`→`0051`; the SFC compiles under `@vue/compiler-sfc`. Applies cleanly |
| `0060` | P10 task 4 taxonomy admin page | Backend behind it is mounted and tested. Applies cleanly |

**The stacking was the real risk, and it behaved exactly as described.** `0056`
sat on `0055`, so `0055`'s wrong import block propagated into it and both failed
together — one root cause, two failures. The `0053`→`0054` stack did not need a
fix-up, so nothing cascaded there.
...
others should have used — and the pattern now superseded by simply extracting the
real file from the image and diffing against it, which is how the two failures
were fixed.

**Two upstream APIs inside `0057` remain outright guesses:** the Vuex
conversation-list getter names, and the response shape of
`GET /api/v1/accounts/:id/conversations`. Applying cleanly says nothing about
either — a patch can apply perfectly and still call a getter that does not exist.
Both fail toward a **visible degraded indicator** rather than toward silence, so a
wrong guess is noticeable and fixable by editing one array, but both must be
confirmed on the first real build.

**What is emphatically NOT closed by this update:** applying is not compiling. No
Cloud Build has run, so nothing has yet proved the patched source builds under
vite, and no patch has been seen rendering in a browser.
...
Compounding it: **14 of the 16 `PHONE_*` settings and all eight Twilio
credentials are set in neither `example.env` nor either compose file** (see
`../handover/configuration.md`), so an operator has no way to discover that the
configuration surface exists. A review fix added P11's seven settings to
`example.env` with a description each; the Twilio credentials remain
undiscoverable, which is the more serious half — **no phone or WhatsApp feature
works at all without them, and nothing tells an operator they exist.**

**Three of those settings gate nothing today**, which a reader enabling the
channel needs to know: `PHONE_DTMF_MENU_ENABLED` and
`PHONE_LIVE_TRANSCRIPT_ENABLED` have no consumer, and the recording-retention job
has no scheduler, so `PHONE_RETENTION_JOB_ENABLED` and
`PHONE_RECORDING_RETENTION_DAYS` cause nothing to be deleted. **No call recording
is ever purged**, at any value of either setting — a retention commitment that
looks configured and is not. Blocked-work register §3k.
...
| **`DATA_SCOPED_RBAC_ENABLED`** | **Gates nothing.** `apply_scope_to_filters` and `resolve_user_data_scope` have no callers, there is no FastAPI dependency, and role scopes live in a module dict with no admin surface. The intersection logic is real and proven; the enforcement is absent. Register §3j |
| `PHONE_DTMF_MENU_ENABLED`, `PHONE_LIVE_TRANSCRIPT_ENABLED` | No consumer |
| `PHONE_RETENTION_JOB_ENABLED` | No scheduler — **no recording is ever purged** |

**The count is now eleven, not nine, and two of the newest are the most
consequential.** `DATA_SCOPED_RBAC_ENABLED` is a **data-access** flag that an
operator could reasonably believe restricts who sees what; it does not, and until
a review corrected it, `config.py` and `example.env` both described enforcement
"in the metrics and admin query layers plus Chatwoot-native inbox membership"
that does not exist. **Nobody should represent data scoping as delivered.** And
`PHONE_RETENTION_JOB_ENABLED` is a **retention** flag that looks configured and
deletes nothing — the kind of gap that surfaces in a privacy audit rather than a
test run.

Both were found by the first review P10/P11/P12 ever received (commit `1fc11a2`),
which is itself the point: those three packages landed via a controller rescue
commit after their implementing agents died on API limits, so they reached the
main branch unreviewed. **The review found two unmounted or unconsumed items and
four overclaiming documentation statements in one pass.** P13 and P14 have had no
equivalent review.
...
1. **P11, P12 and P13's flags are not in `FLAGS_ON`.** Those packages never
   reached the step that adds them, so **the on-path of 14 of the 16 `PHONE_*`
...
| R2 | 59-patch fork rebase per upstream release | H | M | Priced as standing effort |
...
| R7 | 9 fork patches unbuilt (all now apply to real upstream; 2 needed rework) | M | M | Cloud Build, amd64, off-VM |
