...
case category/subcategory fields used by Cases (see the Cases chapter).
...
Proton's case category and subcategory fields (used on the Cases page) are
defined here as conversation-level custom attributes, so any conversation
can be tagged with the same structured category values used in Case
reporting.

### Integrations & automation

Custom attributes back the Cases feature's category/subcategory fields (see
...
SLA Policies define the response and resolution time targets the CRM tracks
per conversation — either a single tenant-wide default, or an override for a
specific inbox.
...
3. Optionally set **Per-channel ACK minutes (JSON)** for a channel-specific
   acknowledgement target, for example `{"whatsapp": 15}`, and a **PIC
   WhatsApp number** to notify.
4. Leave a field empty on an inbox's policy to inherit the tenant default
   instead of setting an inbox-specific value.
5. Click **Save**. The policy applies to conversations on that scope going
   forward; it does not re-evaluate already-closed conversations.

[[SCREENSHOT: ch09-sla-policies | Creating an SLA policy]]

### Example scenario

Proton sets a tenant-wide default of a 1-hour response window and an 8-hour
resolution window, then overrides the WhatsApp inbox specifically with a
30-minute response window, since customers expect a faster first reply on
that channel.
...
which person-in-charge (PIC) to notify for each department, and which email
address represents each dealer, for SLA breaches and escalated
conversations.
...
2. To add a department's PIC, fill in **Department**, **PIC name**, **PIC
   email**, optionally **PIC WhatsApp**, and optionally **CC emails**
   (comma-separated), then click **Add PIC**.
3. To change an existing PIC, click **Edit** on its row, update the fields,
   and click **Save**; or click **Delete** and confirm to remove it.
4. To add a dealer, fill in **Dealer** and **Email** under the Dealers
   section, then click **Add dealer**. Edit or delete a dealer row the same
   way as a PIC row.
5. Changes take effect immediately for new escalations — there is no
...
[[SCREENSHOT: ch09-escalation-routing | Editing the PIC contact for a department and the dealer directory]]
...
chapter) uses to attribute a handoff to a specific dealer.
